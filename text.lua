...
-- AUTO CLICK + BAT ATTACK - COMPLETO
-- Copia TODO este código y reemplaza desde "-- INSTANT KILL BRAINROTS" hasta "PlantsPage.CanvasSize"

local InstantKillSection = Instance.new("Frame")
InstantKillSection.Size = UDim2.new(1, -6, 0, 130)
InstantKillSection.BackgroundColor3 = Color3.fromRGB(35, 30, 45)
InstantKillSection.BorderSizePixel = 0
InstantKillSection.Parent = PlantsPage

local InstantKillCorner = Instance.new("UICorner")
InstantKillCorner.CornerRadius = UDim.new(0, 7)
InstantKillCorner.Parent = InstantKillSection

local InstantKillStroke = Instance.new("UIStroke")
InstantKillStroke.Color = Color3.fromRGB(255, 50, 50)
InstantKillStroke.Thickness = 2
InstantKillStroke.Transparency = 0.5
InstantKillStroke.Parent = InstantKillSection

local InstantKillTitle = Instance.new("TextLabel")
InstantKillTitle.Size = UDim2.new(0.7, -16, 0, 25)
InstantKillTitle.Position = UDim2.new(0, 8, 0, 8)
InstantKillTitle.BackgroundTransparency = 1
InstantKillTitle.Text = "Auto Click + Bat Attack"
InstantKillTitle.TextColor3 = Color3.fromRGB(255, 100, 100)
InstantKillTitle.Font = Enum.Font.GothamBold
InstantKillTitle.TextSize = 15
InstantKillTitle.TextXAlignment = Enum.TextXAlignment.Left
InstantKillTitle.Parent = InstantKillSection

local InstantKillDesc = Instance.new("TextLabel")
InstantKillDesc.Size = UDim2.new(1, -16, 0, 40)
InstantKillDesc.Position = UDim2.new(0, 8, 0, 35)
InstantKillDesc.BackgroundTransparency = 1
InstantKillDesc.Text = "Auto-click 0.1s | Rango 50 studs | Auto-equipa bate | Sin cooldown"
InstantKillDesc.TextColor3 = Color3.fromRGB(200, 200, 220)
InstantKillDesc.Font = Enum.Font.Gotham
InstantKillDesc.TextSize = 11
InstantKillDesc.TextXAlignment = Enum.TextXAlignment.Left
InstantKillDesc.TextYAlignment = Enum.TextYAlignment.Top
InstantKillDesc.TextWrapped = true
InstantKillDesc.Parent = InstantKillSection

local InstantKillToggle = Instance.new("TextButton")
InstantKillToggle.Size = UDim2.new(0, 65, 0, 30)
InstantKillToggle.Position = UDim2.new(1, -73, 0, 8)
InstantKillToggle.BackgroundColor3 = Color3.fromRGB(60, 50, 70)
InstantKillToggle.Text = "OFF"
InstantKillToggle.TextColor3 = Color3.fromRGB(255, 255, 255)
InstantKillToggle.Font = Enum.Font.GothamBold
InstantKillToggle.TextSize = 13
InstantKillToggle.BorderSizePixel = 0
InstantKillToggle.Parent = InstantKillSection

local InstantKillToggleCorner = Instance.new("UICorner")
InstantKillToggleCorner.CornerRadius = UDim.new(0, 7)
InstantKillToggleCorner.Parent = InstantKillToggle

local InstantKillIndicator = Instance.new("Frame")
InstantKillIndicator.Size = UDim2.new(0, 10, 0, 10)
InstantKillIndicator.Position = UDim2.new(0, 10, 0, 10)
InstantKillIndicator.BackgroundColor3 = Color3.fromRGB(255, 70, 70)
InstantKillIndicator.BorderSizePixel = 0
InstantKillIndicator.Parent = InstantKillToggle

local InstantKillIndicatorCorner = Instance.new("UICorner")
InstantKillIndicatorCorner.CornerRadius = UDim.new(1, 0)
InstantKillIndicatorCorner.Parent = InstantKillIndicator

local AttackSpeedLabel = Instance.new("TextLabel")
AttackSpeedLabel.Size = UDim2.new(0, 200, 0, 25)
AttackSpeedLabel.Position = UDim2.new(0, 8, 1, -33)
AttackSpeedLabel.BackgroundTransparency = 1
AttackSpeedLabel.Text = "Estado: Desactivado"
AttackSpeedLabel.TextColor3 = Color3.fromRGB(150, 150, 170)
AttackSpeedLabel.Font = Enum.Font.GothamBold
AttackSpeedLabel.TextSize = 12
AttackSpeedLabel.TextXAlignment = Enum.TextXAlignment.Left
AttackSpeedLabel.Parent = InstantKillSection

local RangeLabel = Instance.new("TextLabel")
RangeLabel.Size = UDim2.new(0, 150, 0, 20)
RangeLabel.Position = UDim2.new(1, -158, 1, -28)
RangeLabel.BackgroundTransparency = 1
RangeLabel.Text = "Clicks: 0/s"
RangeLabel.TextColor3 = Color3.fromRGB(100, 200, 255)
RangeLabel.Font = Enum.Font.Gotham
RangeLabel.TextSize = 11
RangeLabel.TextXAlignment = Enum.TextXAlignment.Right
RangeLabel.Parent = InstantKillSection
...
local function GetPlayerBat()
    ...
    for _, item in pairs(character:GetChildren()) do
        if item:IsA("Tool") then
            ...
            if name:find("bat") or name:find("fluted") or name:find("weapon") then
                return item
            end
        end
    end
...
local function AttackNearbyEnemies()
    ...
    local bat = GetPlayerBat()
    if not bat then 
        AttackSpeedLabel.Text = "No se encontro bate"
        AttackSpeedLabel.TextColor3 = Color3.fromRGB(255, 150, 50)
        return 
    end
            ...
            if enemyHum and enemyHum.Health > 0 then
                ...
                if enemyRoot and RootPart then
                    ...
                    if distance <= AttackRange then
                        ...
                        for _, remote in pairs(bat:GetDescendants()) do
                            if remote:IsA("RemoteEvent") then
                                ...
                                if remoteName:find("attack") or remoteName:find("hit") or remoteName:find("damage") or remoteName:find("swing") then
                                    pcall(function()
                                        ...
                                        remote:FireServer(enemy, enemyRoot)
                                        remote:FireServer({Target = enemy, Part = enemyRoot})
                                    end)
                                end
                            end
    ...
    if attackCount > 0 then
        AttackSpeedLabel.Text = string.format("Atacando: %d enemigos", attackCount)
        AttackSpeedLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
    else
        AttackSpeedLabel.Text = "Buscando enemigos..."
        AttackSpeedLabel.TextColor3 = Color3.fromRGB(100, 200, 255)
    end
end
...
task.spawn(function()
    while task.wait(1) do
        if AutoAttackEnabled then
            RangeLabel.Text = string.format("Clicks: %d/s", ClickCount)
            ClickCount = 0
        end
    end
end)
...
InstantKillToggle.MouseButton1Click:Connect(function()
    ...
    InstantKillToggle.Text = AutoAttackEnabled and "ON" or "OFF"
    InstantKillToggle.BackgroundColor3 = AutoAttackEnabled and Color3.fromRGB(255, 50, 50) or Color3.fromRGB(60, 50, 70)
    InstantKillIndicator.BackgroundColor3 = AutoAttackEnabled and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 70, 70)
    InstantKillStroke.Transparency = AutoAttackEnabled and 0 or 0.5
    
    if AutoAttackEnabled then
        ShowNotification("Auto Click + Attack Activado!", Color3.fromRGB(255, 100, 100))
        AttackSpeedLabel.Text = "Activado - Auto-clicking..."
        AttackSpeedLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
        RangeLabel.Text = "Clicks: 0/s"
        ClickCount = 0
        ...
        ShowNotification("Auto Click + Attack Desactivado", Color3.fromRGB(100, 255, 100))
        AttackSpeedLabel.Text = "Estado: Desactivado"
        AttackSpeedLabel.TextColor3 = Color3.fromRGB(150, 150, 170)
        RangeLabel.Text = "Clicks: 0/s"
...
InstantKillToggle.MouseEnter:Connect(function()
    TweenService:Create(InstantKillToggle, TweenInfo.new(0.2), {Size = UDim2.new(0, 70, 0, 32)}):Play()
end)

InstantKillToggle.MouseLeave:Connect(function()
    TweenService:Create(InstantKillToggle, TweenInfo.new(0.2), {Size = UDim2.new(0, 65, 0, 30)}):Play()
end)
...
-- NOCLIP PARA MÓVIL Y PC - CON CONTROLES EN PANTALLA
local NoClipCFrameLoop = nil
local NoClipControlsVisible = false

-- Crear controles táctiles para móvil
local MobileControls = Instance.new("Frame")
MobileControls.Size = UDim2.new(0, 180, 0, 180)
MobileControls.Position = UDim2.new(1, -200, 1, -200)
MobileControls.BackgroundTransparency = 1
MobileControls.Visible = false
MobileControls.ZIndex = 50
MobileControls.Parent = ScreenGui

local function CreateDirectionButton(text, position, direction)
    local btn = Instance.new("TextButton")
    ...
    return btn, function() return isPressed end
end

-- Crear botones direccionales
local upBtn, isUpPressed = CreateDirectionButton("↑", UDim2.new(0, 65, 0, 0), "up")
local downBtn, isDownPressed = CreateDirectionButton("↓", UDim2.new(0, 65, 0, 130), "down")
local leftBtn, isLeftPressed = CreateDirectionButton("←", UDim2.new(0, 0, 0, 65), "left")
local rightBtn, isRightPressed = CreateDirectionButton("→", UDim2.new(0, 130, 0, 65), "right")
local forwardBtn, isForwardPressed = CreateDirectionButton("W", UDim2.new(0, 65, 0, 35), "forward")
local backBtn, isBackPressed = CreateDirectionButton("S", UDim2.new(0, 65, 0, 95), "back")

-- Botón central para info
local centerInfo = Instance.new("TextLabel")
centerInfo.Size = UDim2.new(0, 50, 0, 50)
centerInfo.Position = UDim2.new(0, 65, 0, 65)
centerInfo.BackgroundColor3 = Color3.fromRGB(25, 22, 35)
centerInfo.BackgroundTransparency = 0.5
centerInfo.Text = "NC"
centerInfo.TextColor3 = Color3.fromRGB(100, 255, 100)
centerInfo.Font = Enum.Font.GothamBold
centerInfo.TextSize = 16
centerInfo.BorderSizePixel = 0
centerInfo.ZIndex = 51
centerInfo.Parent = MobileControls

local centerCorner = Instance.new("UICorner")
centerCorner.CornerRadius = UDim.new(1, 0)
centerCorner.Parent = centerInfo
...
NoClipToggle.MouseButton1Click:Connect(function()
    ...
    if NoClipEnabled then
        ...
        ShowNotification("NoClip Activado - Usa los controles en pantalla", Color3.fromRGB(100, 255, 100))
        ...
        NoClipCFrameLoop = RunService.Heartbeat:Connect(function()
            if NoClipEnabled and RootPart then
                local camera = workspace.CurrentCamera
                local moveDirection = Vector3.new(0, 0, 0)
                
                -- Controles PC (WASD)
                if UIS:IsKeyDown(Enum.KeyCode.W) or isForwardPressed() then
                    moveDirection = moveDirection + (camera.CFrame.LookVector)
                end
                if UIS:IsKeyDown(Enum.KeyCode.S) or isBackPressed() then
                    moveDirection = moveDirection - (camera.CFrame.LookVector)
                end
                if UIS:IsKeyDown(Enum.KeyCode.A) or isLeftPressed() then
                    moveDirection = moveDirection - (camera.CFrame.RightVector)
                end
                if UIS:IsKeyDown(Enum.KeyCode.D) or isRightPressed() then
                    moveDirection = moveDirection + (camera.CFrame.RightVector)
                end
                
                -- Subir/Bajar (Space/Shift o botones táctiles)
                if UIS:IsKeyDown(Enum.KeyCode.Space) or isUpPressed() then
                    moveDirection = moveDirection + Vector3.new(0, 1, 0)
                end
                if UIS:IsKeyDown(Enum.KeyCode.LeftShift) or isDownPressed() then
                    moveDirection = moveDirection - Vector3.new(0, 1, 0)
                end
                
                -- Aplicar movimiento
                if moveDirection.Magnitude > 0 then
                    local speed = Humanoid.WalkSpeed / 16
                    RootPart.CFrame = RootPart.CFrame + (moveDirection.Unit * speed * 0.3)
                end
            end
        end)
        ...
        MobileControls.Visible = false
        ShowNotification("NoClip Desactivado", Color3.fromRGB(255, 100, 100))
        
        if NoClipConnection then
            NoClipConnection:Disconnect()
            NoClipConnection = nil
        end
        if NoClipCFrameLoop then
            NoClipCFrameLoop:Disconnect()
            NoClipCFrameLoop = nil
        end
